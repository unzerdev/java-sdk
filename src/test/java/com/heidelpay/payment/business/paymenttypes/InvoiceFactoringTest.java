...
public class InvoiceFactoringTest extends AbstractPaymentTest {
	...
	public void testChargeType() throws HttpCommunicationException, MalformedURLException, ParseException {
		...
		invoice.charge(BigDecimal.TEN, Currency.getInstance("EUR"), new URL("https://www.meinShop.de"), getMaximumCustomerSameAddress(getRandomInvoiceId()), getMaxTestBasket());		
	}

	@Test
	public void testChargeTypeWithInvoiceId() throws HttpCommunicationException, MalformedURLException, ParseException {
		InvoiceFactoring invoice = createInvoiceFactoring();
		invoice.charge(BigDecimal.TEN, Currency.getInstance("EUR"), new URL("https://www.meinShop.de"), getMaximumCustomerSameAddress(getRandomInvoiceId()), getMaxTestBasket(), getRandomInvoiceId());		
	}
	...
	public void testShipmentInvoiceFactoringTypeWithInvoiceId() throws HttpCommunicationException, MalformedURLException, ParseException {
		...
		Charge charge = getHeidelpay().charge(new BigDecimal(100), Currency.getInstance("EUR"), createInvoiceFactoring().getId(), new URL("https://www.meinShop.de"), createMaximumCustomerSameAddress().getId() , basket.getId(), false);
